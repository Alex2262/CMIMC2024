...
class NeuralNetwork:
    ...
    def feedforward(self, inputs):
        ...
        for i in range(self.output_size):
            if inputs[i] == 1 or inputs[96 + i] == 1 or inputs[96 * 2 + i] == 1:
    ...
    def update_weights(self, gradient):
        ...
        for i in range(self.output_size):
            for j in range(self.input_size):
                self.weights[i][j] -= gradient.weights[i][j]
                self.weights[i][j] = clamp(self.weights[i][j], self.lower_bound, self.upper_bound)

        for i in range(self.output_size):
            self.bias[i] -= gradient.bias[i]
            self.bias[i] = clamp(self.bias[i], self.lower_bound, self.upper_bound)
...
class Gradient:
    def __init__(self, input_size, output_size):
        ...
        self.weights = [[0 for _ in range(input_size)] for _ in range(output_size)]
...
class RL:
    ...
    def backprop(self, gradient, inputs, actual, lr):
        ...
        # Update weights and biases from input to output
